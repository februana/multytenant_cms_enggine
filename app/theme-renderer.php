...
function render_shared_section_block(array $config, string $sectionId, array $shared): string {
    ...
    $sectionStyle = (string)($sectionStyles[$sectionId] ?? '');
    ...
    switch ($sectionId) {
        case 'countdown':
            return '<section id="countdown-section" class="section countdown-section" ' . $sectionStyle . '><div class="section-head"><p class="label">Menuju Hari Bahagia</p><h2>Hitung Mundur Pernikahan</h2></div><div id="countdown" class="countdown" data-countdown="' . escape_html($countdownTarget) . '" aria-label="Hitung mundur acara"><div><strong>00</strong><span>Hari</span></div><div><strong>00</strong><span>Jam</span></div><div><strong>00</strong><span>Menit</span></div><div><strong>00</strong><span>Detik</span></div></div></section>';
        case 'guest_intro':
...
function render_theme_layout(array $config, array $shared): string {
    ...
    $customVisuals = function_exists('theme_visual_values_for_config') ? theme_visual_values_for_config($config, 'custom') : [];
    $customSectionKeys = [
        'undangan' => 'section_background_home', 'countdown' => 'section_background_event', 'acara' => 'section_background_event',
        'cerita' => 'section_background_story', 'galeri' => 'section_background_gallery', 'lokasi' => 'section_background_location',
        'amplop' => 'section_background_gift', 'rsvp' => 'section_background_rsvp'
    ];
    $customSectionStyles = [];
    foreach ($customSectionKeys as $sectionId => $visualKey) {
        $path = trim((string)($customVisuals[$visualKey] ?? ''));
        if ($path === '') continue;
        $styleValue = 'background-image:linear-gradient(rgba(255,250,245,.78),rgba(255,250,245,.88)),' . theme_visual_css_url($path) . ';background-size:cover;background-position:center;background-repeat:no-repeat;';
        $customSectionStyles[$sectionId] = 'style="' . escape_html($styleValue) . '"';
    }
    $shared['sectionStyles'] = $customSectionStyles;
    $parts = [];
