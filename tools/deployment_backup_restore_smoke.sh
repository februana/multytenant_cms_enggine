...
make_base_app() {
    ...
    printf 'user-db' > "$app/database.sqlite"
    printf 'user-env' > "$app/.env"
...
cmp "$APP/config.json" "$RESTORE_APP/config.json"
cmp "$APP/database.sqlite" "$RESTORE_APP/database.sqlite"
...
[ -d "$RESTORE_APP/uploads/theme-assets/dewankl" ] || fail 'restore did not bootstrap legacy-missing Theme Asset directory'
[ -d "$RESTORE_APP/uploads/theme-assets/custom" ] || fail 'restore did not bootstrap Custom Theme Asset directory'
pass 'valid backup restores state, WebDAV credentials, hidden files, spaces, and missing canonical directories'
...
[ -f "$LEGACY_RESTORE/config.json" ] || fail 'legacy config was not restored'
[ -f "$LEGACY_RESTORE/uploads/gallery/legacy.webp" ] || fail 'legacy Gallery media was not restored'
[ -d "$LEGACY_RESTORE/uploads/theme-assets/pawiwahan" ] || fail 'legacy restore did not create Theme Assets directories'
pass 'legacy archive without Theme Assets remains compatible'
...
[ "$(cat "$UPDATE_APP/config.json")" = user-config ] || fail 'successful update replaced config.json'
...
[ -d "$UPDATE_APP/uploads/theme-assets/dewankl" ] || fail 'successful update did not bootstrap Theme Asset directories'
