...
$siteTitle = escape_html($config['site']['title'] ?? ($semanticNames['bride_full_name'] . ' & ' . $semanticNames['groom_full_name']));
...
$openingGreeting = render_preserved_text(theme_opening_greeting($config, 'rainier'));
...
$eventData = [
    'event' => ['title' => html_entity_decode($brideNickname . ' & ' . $groomNickname, ENT_QUOTES, 'UTF-8'), 'subtitle' => $openingRaw, 'description' => $descriptionRaw],
...
<body>
    ...
    <main id="app" aria-live="polite">
        <?php if (theme_section_enabled($config, $presetKey, 'hero')): ?><section class="hero"><p id="guest-greeting" class="guest-greeting">Kepada Yth. <?php echo $guestLabel; ?></p><h1 id="event-title"><?php echo $brideNickname; ?> &amp; <?php echo $groomNickname; ?></h1><p class="opening-greeting" id="opening-greeting"><?php echo $openingGreeting; ?></p><p class="subtitle" id="event-subtitle"><?php echo $openingText; ?></p><div class="hero-meta"><?php if (!empty($config['schedule']['countdown_target'])): ?><div id="countdown" class="countdown" aria-live="polite"><span class="countdown-label">Acara dimulai dalam</span><div class="countdown-grid"><div class="countdown-unit"><strong id="cd-days">0</strong><span>Hari</span></div><div class="countdown-unit"><strong id="cd-hours">00</strong><span>Jam</span></div><div class="countdown-unit"><strong id="cd-minutes">00</strong><span>Menit</span></div><div class="countdown-unit"><strong id="cd-seconds">00</strong><span>Detik</span></div></div></div><?php endif; ?><div id="calendar-actions"><a id="google-calendar-link" target="_blank" rel="noopener" href="<?php echo escape_html($calendarLink); ?>">Tambahkan ke Google Kalender</a></div></div><?php if (theme_section_enabled($config, $presetKey, 'rsvp')): ?><a href="#rsvp" class="primary-btn">Konfirmasi Kehadiran</a><?php endif; ?></section><?php endif; ?>
