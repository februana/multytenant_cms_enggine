    ...
    <div class="container">
      ...
      <div class="collapse navbar-collapse" id="navbarNav">
        <ul class="navbar-nav ms-auto">
          <?php foreach ([['home', '#home', 'Beranda'], ['about', '#about', 'Tentang Kami'], ['gallery', '#galeri', 'Galeri'], ['location', '#lokasi', 'Lokasi Upacara'], ['messages', '#pesan', 'Pesan & Doa']] as $nav): if (!$sectionEnabled($nav[0])) continue; ?>
...
<div id="carouselExampleCaptions" class="carousel slide" data-bs-ride="carousel">
  ...
  <div class="carousel-inner">
    <?php foreach ([$coverUrl, $brideUrl, $groomUrl] as $index => $image): ?><div class="carousel-item <?php echo $index === 0 ? 'active' : ''; ?>"><img src="<?php echo escape_html($image); ?>" class="d-block w-100 pawiwahan-media-cover" alt="Pawiwahan slide <?php echo $index + 1; ?>"><div class="carousel-caption d-md-block text-info"><h5>Pawiwahan</h5><p><?php echo $brideNickname; ?> &amp; <?php echo $groomNickname; ?></p></div></div><?php endforeach; ?>
  </div>
  ...
  <button class="carousel-control-next" type="button" data-bs-target="#carouselExampleCaptions" data-bs-slide="next"><span class="carousel-control-next-icon" aria-hidden="true"></span><span class="visually-hidden">Berikutnya</span></button>
</div>
<?php endif; ?>
<main>
...
<section class="card" id="galeri"><div class="container col-md-6"><div class="row text-center mb-3"><div class="col"><h2>Kisah Kami</h2></div></div><?php if (!$galleryUrls): ?><p class="pawiwahan-gallery-empty text-center">Belum ada foto galeri yang ditambahkan.</p><?php else: foreach ($carouselGroups as $groupIndex => $group): ?><div class="row" style="display:block"><div class="col-md mb-3"><div class="card-"><div id="carouselExampleControls<?php echo $groupIndex; ?>" class="carousel slide" data-bs-ride="carousel"><div class="carousel-inner"><?php foreach ($group as $imageIndex => $image): ?><div class="carousel-item <?php echo $imageIndex === 0 ? 'active' : ''; ?>"><img src="<?php echo escape_html($image); ?>" class="d-block w-100" alt="Foto galeri <?php echo $groupIndex + 1; ?>-<?php echo $imageIndex + 1; ?>" loading="lazy"></div><?php endforeach; ?></div><button class="carousel-control-prev" type="button" data-bs-target="#carouselExampleControls<?php echo $groupIndex; ?>" data-bs-slide="prev"><span class="carousel-control-prev-icon" aria-hidden="true"></span><span class="visually-hidden">Sebelumnya</span></button><button class="carousel-control-next" type="button" data-bs-target="#carouselExampleControls<?php echo $groupIndex; ?>" data-bs-slide="next"><span class="carousel-control-next-icon" aria-hidden="true"></span><span class="visually-hidden">Berikutnya</span></button></div></div></div></div><?php endforeach; endif; ?></div></section>
<?php endif; ?>
<?php if ($sectionEnabled('location')): ?><section id="lokasi" class="card"><div class="container col-md-6"><div class="row text-center mb-3 mt-3"><div class="col"><h2>Lokasi Upacara</h2></div></div><?php if ($mapsEmbed !== ''): ?><div class="ratio ratio-1x1"><iframe src="<?php echo $mapsEmbed; ?>" title="Lokasi upacara" loading="lazy"></iframe></div><?php endif; ?><div class="row text-center mb-3 mt-3"><div class="col"><h5 class="card-title"><?php echo $venue; ?></h5><p><?php echo $address; ?></p><?php if ($mapsUrl !== ''): ?><a href="<?php echo $mapsUrl; ?>" target="_blank" rel="noopener" class="btn btn-outline-success"><i class="bi bi-geo-alt-fill" style="font-size:1rem"></i> Lihat Google Maps</a><?php endif; ?></div></div></div></section><?php endif; ?>
<?php if ($sectionEnabled('gift')): ?>
<section id="gift" class="card"><div class="container col-md-6"><div class="row text-center mb-3 mt-3"><div class="col"><h2>Amplop Digital</h2><p>Bagi keluarga dan sahabat yang ingin memberikan hadiah, silakan klik tombol di bawah. Terima kasih.</p></div></div><div class="row text-center mb-3 mt-0"><div class="col"><button id="pawiwahan-gift-trigger" type="button" class="btn btn-outline-success" data-bs-toggle="modal" data-bs-target="#pawiwahanGiftModal" aria-controls="pawiwahanGiftModal" aria-expanded="false"><i class="bi bi-gift-fill"></i> Amplop Digital</button></div></div></div></section>
<div class="modal fade" id="pawiwahanGiftModal" tabindex="-1" aria-labelledby="pawiwahanGiftModalLabel" aria-hidden="true"><div class="modal-dialog"><div class="modal-content"><div class="modal-header"><h5 class="modal-title" id="pawiwahanGiftModalLabel">Jika memberi adalah ungkapan tanda kasih Anda, Anda dapat mengirim amplop digital secara transfer pada akun di bawah ini.</h5></div><div class="modal-body"><p><?php echo escape_html((string)($config['gift']['bank'] ?? '')); ?> — <?php echo escape_html((string)($config['gift']['account_holder'] ?? '')); ?></p><div class="pawiwahan-gift-account"><div class="pawiwahan-gift-number" id="pawiwahanGiftNumber"><?php echo escape_html((string)($config['gift']['account_number'] ?? '')); ?></div><button id="pawiwahanGiftCopy" type="button" class="material-symbols-outlined" data-copy="<?php echo escape_html((string)($config['gift']['account_number'] ?? '')); ?>" aria-label="Salin nomor rekening"><i class="bi bi-clipboard-check"></i></button><span id="pawiwahanGiftCopyStatus" class="popuptext" role="status" aria-live="polite"></span></div><p><?php echo escape_html((string)($config['gift']['e_wallet_label'] ?? '')); ?>: <?php echo escape_html((string)($config['gift']['e_wallet_number'] ?? '')); ?></p><?php if ($qrisUrl !== ''): ?><div class="pawiwahan-qris" style="margin-top:16px;text-align:center;"><strong>QRIS</strong><img src="<?php echo escape_html($qrisUrl); ?>" alt="QRIS untuk tanda kasih" loading="lazy" decoding="async" style="display:block;width:min(100%,220px);aspect-ratio:1/1;object-fit:contain;background:#fff;padding:10px;margin:10px auto 0;border-radius:10px;"></div><?php endif; ?></div><div class="modal-footer"><button type="button" class="btn btn-secondary" data-bs-dismiss="modal">Tutup</button></div></div></div></div>
<?php endif; ?>
